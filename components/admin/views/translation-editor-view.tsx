...
import { Bot, CheckCircle2, ChevronLeft, ChevronRight, CircleDollarSign, CloudUpload, Gauge, Save, TriangleAlert } from "lucide-react";
...
export function TranslationEditorView({ data, canPublish }: { data: Data; canPublish: boolean }) {
  ...
  return <div className="grid gap-4">
    ...
    {message ? <TranslationNotice role="status" tone="success" title="อัปเดตเรียบร้อย" description={message} /> : null}
    {!latest ? <TranslationNotice tone="warning" title="ตอนนี้ยังไม่มีฉบับแปล" description="กลับไปหน้ารายการตอน เลือกตอนนี้แล้วเริ่มแปลด้วย AI หรือพิมพ์คำแปลในช่องด้านล่างและบันทึกเป็นฉบับแรก" /> : latest.status === "APPROVED" && latest.origin === "AI" ? <TranslationNotice role="status" tone="success" title="คำแปลผ่านการตรวจแล้ว" description={canPublish ? "ตรวจทานรอบสุดท้ายแล้วกดเผยแพร่ได้ทันที" : "งานส่วนของคุณเสร็จแล้ว ผู้ดูแลระบบจะเป็นผู้เผยแพร่ขึ้นหน้าอ่าน"} /> : criticalCount > 0 ? <TranslationNotice tone="warning" title={`ต้องแก้ปัญหาสำคัญ ${criticalCount.toLocaleString("th-TH")} รายการ`} description="เปิดแท็บการตรวจ ใช้คำแนะนำหรือแก้เนื้อหา แล้วบันทึกและตรวจใหม่ก่อนอนุมัติ" /> : null}
    {!data.workspace.novelId && canPublish ? <TranslationNotice tone="info" title="การเผยแพร่ครั้งแรก" description="ระบบจะสร้างหน้านิยายและเชื่อมตอนนี้ให้อัตโนมัติหลังยืนยันเผยแพร่" /> : null}

    <div className="grid gap-3 grid-cols-2 xl:grid-cols-4">
      <TranslationMetric label="ต้นทุน AI โดยประมาณ" value={formatAiCost(data.aiUsage.costMicros, "$0.00")} hint="คำนวณจาก token ทุกครั้งของตอนนี้" icon={CircleDollarSign} tone="brand" />
      <TranslationMetric label="เรียก AI" value={`${data.aiUsage.aiCallCount.toLocaleString("th-TH")} ครั้ง`} hint={`${data.aiUsage.inputTokens.toLocaleString("th-TH")} input tokens`} icon={Bot} tone="neutral" />
      <TranslationMetric label="Cache hit" value={`${data.aiUsage.cacheHitPercent.toLocaleString("th-TH")}%`} hint={`${data.aiUsage.cachedInputTokens.toLocaleString("th-TH")} tokens ใช้ซ้ำ`} icon={Gauge} tone="success" />
      <TranslationMetric label="รายการรอตรวจ" value={unresolvedIssues.length.toLocaleString("th-TH")} hint={criticalCount ? `${criticalCount.toLocaleString("th-TH")} รายการสำคัญ` : "ไม่มีรายการสำคัญ"} icon={TriangleAlert} tone={criticalCount ? "warning" : "neutral"} />
    </div>
    ...
    <div className="grid grid-cols-3 gap-1 rounded-[12px] bg-muted p-1 lg:hidden" role="tablist" aria-label="ส่วนตรวจคำแปล"><Button type="button" role="tab" aria-selected={mobilePane === "SOURCE"} size="sm" variant={mobilePane === "SOURCE" ? "secondary" : "ghost"} onClick={() => setMobilePane("SOURCE")}>ต้นฉบับ</Button><Button type="button" role="tab" aria-selected={mobilePane === "TRANSLATION"} size="sm" variant={mobilePane === "TRANSLATION" ? "secondary" : "ghost"} onClick={() => setMobilePane("TRANSLATION")}>คำแปล</Button><Button type="button" role="tab" aria-selected={mobilePane === "QA"} size="sm" variant={mobilePane === "QA" ? "secondary" : "ghost"} onClick={() => setMobilePane("QA")}>การตรวจ ({unresolvedIssues.length})</Button></div>
