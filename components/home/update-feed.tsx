    ...
    <section aria-label={title}>
      <div className="mb-5 flex items-end justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            <p className="editorial-kicker">LATEST UPDATES / 新着</p>
              ...
              <li key={`${item.novelSlug}-${item.chapter}-${index}`}>
                <article className="group">
                  <Link href={`/novel/${novel.slug}/chapter/${item.chapter}`} prefetch className="block">
                    ...
                    <h3 className="mt-2 line-clamp-2 font-serif text-sm font-semibold leading-[1.55] transition-colors group-hover:text-[var(--brand-primary)]">{novel.thaiTitle}</h3>
                    ...
                    <div className="mt-2 flex items-center justify-between gap-2 text-[11px] text-muted-foreground">
                      ...
                      <span className="tabular shrink-0 text-[var(--brand-primary)]">{item.time}</span>
                    </div>
                  </Link>
                </article>
              </li>
