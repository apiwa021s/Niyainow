...
export function GenrePicker({ items }: { items: { genre: Genre; covers: string[] }[] }) {
  return (
    <section aria-label="เลือกตามแนว" className="render-deferred border-y border-border py-8 sm:py-10">
      <div className="flex flex-col gap-6 lg:grid lg:grid-cols-[280px_1fr] lg:gap-10">
        <div>
          ...
          <Link href="/genres" className="mt-4 inline-flex min-h-11 items-center gap-1 text-sm font-semibold text-[var(--brand-primary)]">ดูหมวดหมู่ทั้งหมด<ArrowRight className="h-4 w-4" /></Link>
        </div>

        <ul className="flex flex-wrap content-start gap-2.5">
          {items.map(({ genre }) => (
            <li key={genre.slug}>
              <Link href={`/genre/${genre.slug}`} className="group inline-flex min-h-11 items-center gap-3 rounded-[6px] border border-border bg-card px-4 text-sm transition-colors hover:border-[var(--brand-primary)] hover:text-[var(--brand-primary)]">
