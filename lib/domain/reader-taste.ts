/**
 * Reader-facing taste taxonomy (Relationship / Setting / Trope / Content Warning / Membership).
 *
...
export function getNovelTaste(novel: { slug: string }): NovelTaste {
  ...
  const warningCount = seed % 4 === 0 ? 1 : 0;
  ...
  return { relationship, setting, tropes, warnings, hasMembership };
}
...
export function matchesTaste(
  novel: { slug: string },
  filters: { relationship?: string; setting?: string; trope?: string },
): boolean {
  const taste = getNovelTaste(novel);
