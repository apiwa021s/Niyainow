...
export async function POST(request: Request) {
  return handleUserRoute(request, { mutation: true, scope: "studio-upload-complete", rateLimit: { limit: 60, windowMs: 60 * 60_000 } }, async (userId) => {
    ...
    try {
      const verified = await verifyUploadedObject({ actor: { id: userId, role: "READER", status: "ACTIVE" }, stagingObjectKey: asset.stagingKey, finalObjectKey: asset.objectKey, expectedContentType: input.contentType, expectedContentLength: input.contentLength, expectedChecksumSha256: asset.metadata.checksumSha256 });
      ...
      try { await deleteB2Object(asset.stagingKey); } catch { /* cleanup is best effort */ }
      throw error;
    }
  });
}
