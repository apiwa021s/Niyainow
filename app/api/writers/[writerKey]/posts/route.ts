...
export async function GET(_request: Request, context: Context) {
  try {
    const [{ writerKey }, user] = await Promise.all([context.params, getCurrentUser()]);
    const posts = await listVisibleWriterPosts(user?.status === "ACTIVE" ? user.id : null, writerKey);
