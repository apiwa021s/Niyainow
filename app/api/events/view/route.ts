...
import { clientIpAddress, rateLimitHeaders, type RateLimitResult } from "@/lib/security/rate-limit";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    limit = await takeDistributedRateLimit(`public-view:${rateFingerprint}`, { limit: 60, windowMs: 60_000 });
    if (!limit.allowed) {
