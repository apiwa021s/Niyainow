...
export default function GenresLoading() {
  return (
    <PageShell className="space-y-7">
      ...
      <header aria-hidden className="border-y border-border py-5 sm:py-6">
        <Skeleton className="h-3 w-44" />
        <Skeleton className="mt-3 h-9 w-60 max-w-full" />
        <Skeleton className="mt-3 h-4 w-[min(560px,90%)]" />
      </header>
      <ul aria-hidden className="grid border-l border-t border-border sm:grid-cols-2 lg:grid-cols-3">
        {Array.from({ length: 9 }, (_, index) => (
          <li key={index} className="min-h-40 border-b border-r border-border p-4 sm:min-h-48 sm:p-5">
